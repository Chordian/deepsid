...
element:"anyfunc"}):Array(b);a.wasmTable=c.table}c.memoryBase||(c.memoryBase=a.STATIC_BASE);c.tableBase||(c.tableBase=0);c=e(c);assert(c,"no binaryen method succeeded.");return c}})();var Y=[function(b){return Math.exp(b)},function(b,c){return Math.pow(b,c)},function(){console.log("FATAL ERROR: no free memory for driver")},function(b){console.log("BRK from:        $"+b.toString(16))},function(){console.log("ERROR: PSID INIT hangs")},function(){console.log("info cannot be retrieved  from corrupt .mus file")}];
K=1024;L=K+292928;ia.push({f:function(){na()}});a.STATIC_BASE=K;a.STATIC_BUMP=292928;var ra=L+=16;L=L+4+15&-16;Q=ra;M=N=A(L);O=M+S;P=A(O);J[Q>>2]=P;a.wasmTableSize=1;a.wasmMaxTableSize=1;a.b={};
