...
if ($csdb_type == 'sid') {
	...
	if (isset($csdb->SID->UsedIn)) {
		...
		$used_by_releases = 
			$user_comments.
			$comment_button.
			'<h3>'.$amount_releases.' release'.($amount_releases > 1 ? 's' : '').' found</h3>'.
...
echo json_encode(array('status' => 'ok', 'html' => $html.'<i><small>Generated using the <a href="https://csdb.dk/webservice/" target="_blank">CSDb web service</a></small></i>', 'count' => $amount_releases));
?>